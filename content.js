...
function oneClick() {
  const companiesInput = Array.from($("input")).filter(el => el.id.match(/react-select-[0-9]+-input/))
  const N = companiesInput.length
  if (companiesInput.length === 0) {
    return "there is nothing to click"
  }
  const companiesId = parseInt(companiesInput[0].id.split("-").filter(el => el.match(/[0-9]+/)))
